...
class ScanView(APIView):
    ...
    def post(self, request):
        ...
        if not barcode or not meal_type:
            return Response({'error': 'barcode and meal_type are required.'}, status=400)
        ...
        if not student:
            MealRegister.objects.create(student=None, meal_type=meal_type, scan_status='denied', scanned_barcode=barcode, scan_date=today)
            return Response({'access':'denied','reason':'not_found','message':f'Barcode "{barcode}" not found in the student database.'}, status=403)

        try:
            app = student.application
            if app.status != 'approved':
                MealRegister.objects.create(student=student, meal_type=meal_type, scan_status='denied', scanned_barcode=barcode, scan_date=today)
                return Response({'access':'denied','reason':'not_approved','message':f'{student.full_name} does not have an approved accommodation application.'}, status=403)
        except AccommodationApplication.DoesNotExist:
            MealRegister.objects.create(student=student, meal_type=meal_type, scan_status='denied', scanned_barcode=barcode, scan_date=today)
            return Response({'access':'denied','reason':'no_application','message':f'{student.full_name} has no accommodation application.'}, status=403)

        already = MealRegister.objects.filter(student=student, meal_type=meal_type, scan_date=today, scan_status='served').first()
        if already:
            MealRegister.objects.create(student=student, meal_type=meal_type, scan_status='duplicate', scanned_barcode=barcode, scan_date=today)
            return Response({'access':'duplicate','message':f'{student.full_name} was already served {meal_type} today at {str(already.scan_time)[:5]}.','served_at':str(already.scan_time)[:5],'student':{'name':student.full_name,'number':student.student_number}}, status=409)

        record = MealRegister.objects.create(student=student, meal_type=meal_type, scan_status='served', scanned_barcode=barcode, scan_date=today)
        return Response({'access':'granted','message':f'{student.full_name} — {meal_type} served.','student':{'name':student.full_name,'number':student.student_number,'hostel':app.hostel.name if app.hostel else None,'gender':student.gender,'condition':student.condition},'record':MealRegisterSerializer(record).data})
...
class DashboardView(APIView):
    ...
    def get(self, request):
        ...
        return Response({
            ...
            'recent_scans':      MealRegisterSerializer(records.order_by('-created_at')[:15], many=True).data,
        })
...
class EligibleView(APIView):
    permission_classes = [IsDiningAdmin]
    def get(self, request):
        apps = AccommodationApplication.objects.filter(status='approved').select_related('student','hostel')
        students = [{'student_number':a.student.student_number,'full_name':a.student.full_name,'hostel':a.hostel.name if a.hostel else None,'gender':a.student.gender,'barcode_id':a.student.barcode_id} for a in apps]
